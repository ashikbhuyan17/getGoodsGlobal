...
export default function OrderPaymentsCard({ payment }: OrderPaymentsCardProps) {
  console.log('🚀 ~ OrderPaymentsCard ~ payment:', payment);
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      <div className="divide-y">
        ...
        />
        <Row label="Amount" value={`৳${payment?.amount ?? 'N/A'}`} />

        <Row
          label="Coupon Discount"
          value={`৳${payment?.coupon_discount ?? 'N/A'}`}
        />
        <Row label="COD Charge" value={`৳${payment?.cod_charge ?? 'N/A'}`} />
        <Row
          label="Shipping Charge"
          value={`৳${payment?.shipping_charge ?? 'N/A'}`}
        />
        <Row
          label="Paid"
          value={`৳${payment?.paid_partial_payment_amount ?? 'N/A'}`}
        />
        {payment?.payment_due_amount != null &&
          Number(payment.payment_due_amount) >= 0 && (

...
function calculateProductTotals(
  ...
  for (const v of variants) {
    ...
    if (v?.product_discount || v?.discount) discount += Number(v.product_discount ?? v.discount ?? 0);
  ...
  for (const v of allVariants) {
    const qty = Number(v?.qty ?? 0);
    const price = Number(v?.sale_price ?? v?.price ?? 0);
    orderTotal += qty * price - Number(v?.product_discount ?? v?.discount ?? 0) + Number(v?.china_courier_charge ?? 0);
  }

  const total = productPrice - discount + chinaCourier;
  const paid = Number(orderData?.paid_partial_payment_amount ?? orderData?.paid_amount ?? orderData?.advance_payment ?? 0);
  const productShare = orderTotal > 0 ? total / orderTotal : 0;
  const discountPct = discount > 0 && productPrice > 0 ? Math.round((discount / productPrice) * 100) : null;

  return { productPrice, discount, chinaCourier, total, paid: paid * productShare, due: total - paid * productShare, discountPct };
}

export default function OrderProductDetails({ products, orderData = {} }: OrderProductDetailsProps) {
  if (!products?.length) {
    return (
  ...
  return (
    <div className="space-y-6">
      {products.map((product, index) => {
        ...
        return (
          <div key={product.product_id ?? index} className="bg-white rounded-lg p-4 md:p-6 border border-gray-200">
                        ...
                        <td className="py-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col gap-1">
                              ...
                              <span className="text-xs text-gray-500">Size: {variant?.size ?? "N/A"}</span>
                            </div>
                          </div>
                        </td>
              ...
              <div className="space-y-2 w-1/2">
                ...
                {totals.due > 0 && (
                  <div className="flex justify-between text-sm font-semibold pt-2">
                    ...
                    <span>৳{Math.round(totals.due)} <span className="text-gray-500 font-normal">+ Shipping Charge</span></span>
                  </div>
                )}
              </div>
            </div>
